...
Go 编写，编译成单一二进制；带 TUI 和多类型自动识别。本目录是 Go 重构版；隔壁 `../create-ctf-poc.py` 是原 Python 版（行为基准，勿动）。
...
`Options = {ForceJDK, ExcludePatterns}` 是 CLI/TUI 透传给 handler 的选项。

### pipeline — 编排层

`Run(jarPath, projectDir, projectName, opts, log)` 是**唯一入口**：
1. 校验文件 → 2. `analyzer.AnalyzeArchive` 取条目+manifest → 3. `analyzer.DetectType` → 4. switch 路由到 handler → 5. `h.Handle(...)`。

`Options = handlers.Options`（类型别名，别再单独定义）。

### tui — 交互层（叶子包）

状态机（`tui.go`）：`stateBrowse` → `stateConfirmFile` → `stateChooseLocation`（仅子目录）→ `stateConfirmMore` → `stateDone`。用 Bubble Tea v1 + bubbles/list；确认组件自写（生态无内置）。`Selection{JarPath, OutputDir}` 是收集结果，退出 TUI 后由 `tui_run.go` 循环调用 `pipeline.Run` 批量生成。

### 入口层

- `main.go` → `runCLI(os.Args[1:])`
- `cli.go`：`flag` 解析 → 无位置参数则 `runTUI()`，否则走 CLI。`intersperseArgs` 重排参数支持 flag 与位置参数混排。
- `tui_run.go`：`runTUI()` 启动 Bubble Tea，收集 `[]Selection` 后批量调 `pipeline.Run`。**非交互终端检测**（`term.IsTerminal`）避免在管道/CI 里卡死。
...
`../web-0.0.1-SNAPSHOT.jar` 是测试基准（Spring Boot 2.3.2 / JDK 8 / 含 commons-collections 3.2.1 + SerialKiller）。改动后用它生成项目，跑 `mvn clean compile` + `mvn exec:java`，再验证 `challenge-classes.jar` 字节一致性。
...
## 项目历史（理解代码注释里的"原 Python 版"）

本 Go 版从 `../create-ctf-poc.py`（Python 版）重构而来。源码注释里偶有「对应原 Python 版 create-ctf-poc.py 的 X」是**溯源说明**（告诉你这个函数对标 Python 版的哪段，便于对照行为基准）。Python 版保留在隔壁不动，作为行为参考与字节级一致性比对的基准。

生成产物里的署名统一是 `ctf-pocgen`（不是 `create-ctf-poc.py`），由模板常量控制；只有源码注释里保留历史引用。
